...
- Opening a modpack's settings dialog no longer crashes with a `NullPointerException` when the modpack's persisted build is null (most commonly after a partial install or manual edit of `installedPacks.json`). The dialog now treats a null build as the default "Recommended" selection and writes that back to `installedPacks.json` immediately, self-healing the corrupt state.
