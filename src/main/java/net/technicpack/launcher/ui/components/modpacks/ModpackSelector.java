...
public class ModpackSelector extends TintablePanel
    implements IModpackContainer, IAuthListener, IRelocalizableResource {
  ...
  public ModpackSelector(
      ResourceLoader resources,
      PackLoader packLoader,
      IPackSource technicSolder,
      ISolderApi solderApi,
      IPlatformApi platformApi,
      IPlatformSearchApi platformSearchApi,
  ...
  private void initComponents() {
    ...
    filterContents =
        new WatermarkTextField(
            resources.getString("launcher.packselector.filter.hotfix"),
    ...
    ((AbstractDocument) filterContents.getDocument())
        .setDocumentFilter(
            new DocumentFilter() {
              @Override
    ...
    scrollPane =
        new JScrollPane(widgetList, VERTICAL_SCROLLBAR_AS_NEEDED, HORIZONTAL_SCROLLBAR_NEVER);
  ...
  public void refreshComplete() {
    ...
    if (selectedWidget == null
        || selectedWidget.getModpack() == null
        || !allModpacks.containsKey(selectedWidget.getModpack().getName())) {
  ...
  static List<ModpackWidget> sortPacks(Collection<ModpackWidget> widgets) {
    ...
    sortedPacks.sort(
        (o1, o2) -> {
          int priorityCompare =
              Integer.compare(o2.getModpack().getPriority(), o1.getModpack().getPriority());
          if (priorityCompare != 0) {
            return priorityCompare;
          } else if (o1.getModpack().getDisplayName() == null
              && o2.getModpack().getDisplayName() == null) {
            return 0;
          } else if (o1.getModpack().getDisplayName() == null) {
            return -1;
            ...
            return o1.getModpack()
                .getDisplayName()
                .compareToIgnoreCase(o2.getModpack().getDisplayName());
          }
        });
    return sortedPacks;
  }
  ...
  protected void addModpackInternal(ModpackModel modpack) {
    final ModpackWidget widget =
        new ModpackWidget(resources, modpack, iconRepo.startImageJob(modpack));
    ...
    if (allModpacks.containsKey(modpack.getName())
        && allModpacks.get(modpack.getName()).isSelected()) {
      selectedWidget = widget;
    }
  ...
  protected void selectWidget(ModpackWidget widget) {
    ...
    Thread thread =
        new Thread("Modpack redownload " + selectedWidget.getModpack().getDisplayName()) {
          @Override
          public void run() {
            try {
              ...
              SwingUtilities.invokeLater(
                  () -> {
                    ...
                    refreshWidget.updateFromPack(
                        iconRepo.startImageJob(refreshWidget.getModpack()));
  ...
  public void forceRefresh() {
    lastFilterContents =
        "THIS IS A TERRIBLE HACK I'M BASICALLY FORCING A REFRESH BUT WITHOUT DOING ANY WORK";
    defaultPacks.clear();
    detectFilterChanges();
    ArrayList<IPackSource> sources = new ArrayList<>(1);
    sources.add(technicSolder);
            ...
            event -> {
              ...
              try {
                ...
                if (host != null
                    ...
                    && (host.equals("www.technicpack.net")
                        || host.equals("technicpack.net")
                        || host.equals("api.technicpack.net"))) {
                  ...
                  if ((fragments.length == 2 && fragments[0].equals("modpack"))
                      || (fragments.length == 3
                          && fragments[0].equals("api")
                          && fragments[1].equals("modpack"))) {
                    ...
                    if (slugMatcher.find()) {
                      ...
                      findMoreWidget.setWidgetData(
                          resources.getString("launcher.packselector.api"));
                      ArrayList<IPackSource> source = new ArrayList<>(1);
                      source.add(new SinglePlatformSource(platformApi, solderApi, slug));
                      currentLoadJob =
                          packLoader.createRepositoryLoadJob(
                              ModpackSelector.this, source, null, false);
                      return;
                    }
                  }
                }
              ...
              currentLoadJob =
                  packLoader.createRepositoryLoadJob(ModpackSelector.this, sources, null, false);
            });
