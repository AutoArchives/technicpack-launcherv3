...
public class ModpackOptionsDialog extends LauncherDialog {
  ...
  protected void selectRecommended() {
    ...
    manualBuildList.setSelectedItem(
        new PackBuildItem(modpack.getRecommendedBuild(), resources, modpack));
  ...
  protected void selectLatest() {
    ...
    manualBuildList.setEnabled(false);
    manualBuildList.setSelectedItem(
        new PackBuildItem(modpack.getLatestBuild(), resources, modpack));
    this.modpack.setBuild(InstalledPack.LATEST);
  }

  protected void selectManual() {
    if (manualBuildList.getItemCount() == 0) return;
    ...
    // Still nothing is selected, and both the currently installed and recommended builds couldn't
    // be selected,
    // so we try to select the first available one (we already know there's at least 1 build
    // available)
  ...
  protected void resetPack() {
    if (modpack.isLocalOnly()) {
      JOptionPane.showMessageDialog(
          this,
          resources.getString("modpackoptions.reinstall.cannotoffline"),
          resources.getString("modpackoptions.reinstall.cannottitle"),
    ...
    int result =
        JOptionPane.showConfirmDialog(
            this,
            resources.getString("modpackoptions.reinstall.confirmtext"),
            resources.getString("modpackoptions.reinstall.confirmtitle"),
  ...
  private void initComponents() {
    ...
    JLabel installFolderLabel =
        new JLabel(resources.getString("modpackoptions.installfolder.text"));
    ...
    RoundedButton openFolder =
        new RoundedButton(resources.getString("modpackoptions.installfolder.open"));
    ...
    openFolder.setEnabled(
        modpack.getInstalledDirectory() != null && modpack.getInstalledDirectory().exists());
    if (modpack.getInstalledDirectory() != null && modpack.getInstalledDirectory().exists()) {
      openFolder.setForeground(UIConstants.COLOR_BUTTON_BLUE);
      openFolder.setHoverForeground(UIConstants.COLOR_BLUE);
    } else {
    ...
    RoundedButton moveFolder =
        new RoundedButton(resources.getString("modpackoptions.installfolder.move"));
    ...
    RoundedButton resetPack =
        new RoundedButton(resources.getString("modpackoptions.reinstall.text"));
  ...
  private void initValues() {
    ...
    if (manualBuildList.getItemCount() == 0) {
      ...
      manualBuildList.addItem(
          new PackBuildItem(
              resources.getString("modpackoptions.version.missing"), resources, modpack));
      manualBuildList.setEnabled(false);
      return;
    }
