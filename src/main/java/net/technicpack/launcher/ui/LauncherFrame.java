...
public class LauncherFrame extends DraggableFrame implements IRelocalizableResource, IAuthListener {
  ...
  public LauncherFrame(
      ...
      final ModpackSelector modpackSelector,
      ...
      final InstalledPackStore packStore,
  ...
  public void selectTab(String tabName) {
    ...
    if (tabName.equalsIgnoreCase(TAB_DISCOVER)) discoverTab.setIsActive(true);
    else if (tabName.equalsIgnoreCase(TAB_MODPACKS)) modpacksTab.setIsActive(true);
  ...
  protected void launchModpack() {
    ...
    if (pack == null
        || (pack.getInstalledPack() == null
            && (pack.getPackInfo() == null || !pack.getPackInfo().isComplete()))) return;
    ...
    // Force a full install (check cache, redownload, unzip files) if we have no current
    // installation of this modpack
    if (installedVersion == null) {
      forceInstall = true;
      requiresInstall = true;
    } else if (pack.getBuild() != null && !pack.isLocalOnly()) {
      // Ask the user if they want to update to the newer version if:
      // 1- the pack build is RECOMMENDED & the recommended version is diff from the installed
      // version
      // 2- the pack build is LATEST & the latest version is diff from the installed version
      // 3- the pack build is neither LATEST nor RECOMMENDED & the pack build is diff from the
      // installed version
      ...
      if (requestInstall) {
        int result =
            JOptionPane.showConfirmDialog(
                this,
                resources.getString("launcher.install.query"),
                resources.getString("launcher.install.query.title"),
  ...
  private static boolean shouldRequestInstall(ModpackModel pack, ModpackVersion installedVersion) {
    ...
    return (wantsRecommended
            && recommendedBuild != null
            && !recommendedBuild.equalsIgnoreCase(installedBuild))
  ...
  /**
   * Called when the installation is complete, either successfully or not. This will hide the
   * progress bar.
   */
  ...
  protected void refreshModpackOptions(ModpackModel model) {
    if (modpackOptionsDialog != null) modpackOptionsDialog.refresh(model);
  }
  ...
  protected void openLauncherOptions() {
    ...
    OptionsDialog dialog =
        new OptionsDialog(
            this, settings, resources, params, javaVersions, fileJavaSource, buildNumber);
  ...
  private void initComponents() {
    ...
    modpackPanel =
        new ModpackInfoPanel(
            ...
            avatarRepo,
            discordApi,
            e -> openModpackOptions((ModpackModel) e.getSource()),
    ...
    newsSelector =
        new NewsSelector(resources, newsInfoPanel, platformApi, avatarRepo, newsCircle, settings);
    ...
    installProgress
        ...
        .setBorder(BorderFactory.createEmptyBorder(3, 20, 2, 20));
    installProgress
        .getOverallProgressBar()
    ...
    if (resources.hasResource("teaser.png")) {
      getRootPane()
          .setGlassPane(
              new SplatPane(
                  modpacksTab, resources.getIcon("teaser.png"), SwingConstants.SOUTH, 5, 0));
  ...
  public void relocalize(ResourceLoader loader) {
    ...
    if (currentTabName != null) selectTab(currentTabName);
  ...
  public void userChanged(IUserType user) {
    if (user == null) this.setVisible(false);
    else {
  ...
  public void setupPlayButtonText(ModpackModel modpack, IUserType user) {
    ...
    if (installer.isCurrentlyRunning()) {
      ...
      playButton.setIcon(
          new ImageIcon(
              resources.colorImage(
                  resources.getImage("play_button.png"), UIConstants.COLOR_BUTTON_BLUE)));
      playButton.setHoverIcon(
          new ImageIcon(
              resources.colorImage(resources.getImage("play_button.png"), UIConstants.COLOR_BLUE)));
    } else {
      if (isUserOffline) {
      ...
      playButton.setIcon(
          new ImageIcon(
              resources.colorImage(
                  resources.getImage("download_button.png"), UIConstants.COLOR_BUTTON_BLUE)));
      playButton.setHoverIcon(
          new ImageIcon(
              resources.colorImage(
                  resources.getImage("download_button.png"), UIConstants.COLOR_BLUE)));
    }
  }
}
